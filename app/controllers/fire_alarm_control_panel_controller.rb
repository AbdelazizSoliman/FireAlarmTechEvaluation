class FireAlarmControlPanelController < ApplicationController
  before_action :set_project_and_system_and_subsystem
      ...
      def create
        @fire_alarm_control_panel = @subsystem.fire_alarm_control_panels.build(fire_alarm_control_panel_params)

        if @fire_alarm_control_panel.save
          redirect_to project_system_path(@project, @system), notice: "Fire Alarm Control Panel created successfully."
        else
          render :new
        end
      end
  ...
  def set_project_and_system_and_subsystem
    @project = Project.find(params[:project_id])
    @system = @project.systems.find(params[:system_id])
